...
if __name__ == "__main__":
    ...
    if vis_human:
        human_obj_files = sorted(glob.glob(osp.join(human_obj_dir, "*.obj")),
                                key=lambda x: int(osp.basename(x).split('.')[0]))

    model_obj_files = sorted(glob.glob(osp.join(model_obj_dir, "*.obj")),
                             key=lambda x: int(osp.basename(x).split('.')[0]))
    ...
    for idx in tbar:
        ...
        # img = np.asarray(visualizer.capture_screen_float_buffer(True)).copy()
